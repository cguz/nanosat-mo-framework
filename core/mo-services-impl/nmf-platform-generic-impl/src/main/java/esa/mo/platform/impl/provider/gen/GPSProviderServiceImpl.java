...
public class GPSProviderServiceImpl extends GPSInheritanceSkeleton
    ...
    @Override
    public LongList listNearbyPosition(IdentifierList names,
    ...
    @Override
    public void removeNearbyPosition(LongList objInstIds,
            MALInteraction interaction) throws MALInteractionException, MALException {
    ...
    @Override
    public void getPositionAndVelocity(GetPositionAndVelocityInteraction interaction)
            throws MALInteractionException, MALException {
        boolean useTLEpropagation = false;
        if (!adapter.isUnitAvailable()) {
            if (isTLEFallbackEnabled) {
    ...
    private Position getTLEPropagatedPosition() {
        ...
        OneAxisEllipsoid earth = new OneAxisEllipsoid(Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                Constants.WGS84_EARTH_FLATTENING, ecf);
        GeodeticPoint satLatLonAlt = earth.transform(state.getPVCoordinates().getPosition(),
                FramesFactory.getEME2000(), state.getDate());

        PositionExtraDetails extraDetails = new PositionExtraDetails(
                new Time(targetDate.getTimeInMillis()),
                0, 0, 0.0f, 0.0f, PositionSourceType.TLE);

        return new Position((float) satLatLonAlt.getLatitude(),
                (float) satLatLonAlt.getLongitude(),
    ...
    private SpacecraftState getSpacecraftState(Calendar targetDate) {
        ...
        return propagator.propagate(new AbsoluteDate(
